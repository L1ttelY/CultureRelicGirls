...
namespace WeChatWASM
{
    ...
    public class UnityUtil
    {
        ...
        public static WXTextureReplacerScriptObject GetTextureEditorCacheConf()
        {
            var path = "Assets/WX-WASM-SDK/Editor/TextureEditor/CacheConfig.asset";
            var config = AssetDatabase.LoadAssetAtPath(path, typeof(WXTextureReplacerScriptObject)) as WXTextureReplacerScriptObject;
            if (config == null)
            {
                AssetDatabase.CreateAsset(EditorWindow.CreateInstance<WXTextureReplacerScriptObject>(), path);
                config = AssetDatabase.LoadAssetAtPath(path, typeof(WXTextureReplacerScriptObject)) as WXTextureReplacerScriptObject;
            }
            return config;
        }

        public static WXTextureFileCacheScriptObject GetTextureFileCachedMd5Conf()
        {
            var path = "Assets/WX-WASM-SDK/Editor/TextureEditor/CachedMd5Config.asset";
            var config = AssetDatabase.LoadAssetAtPath(path, typeof(WXTextureFileCacheScriptObject)) as WXTextureFileCacheScriptObject;
            if (config == null)
            {
                AssetDatabase.CreateAsset(EditorWindow.CreateInstance<WXTextureFileCacheScriptObject>(), path);
                config = AssetDatabase.LoadAssetAtPath(path, typeof(WXTextureFileCacheScriptObject)) as WXTextureFileCacheScriptObject;
            }
            return config;
        }
