...
namespace WeChatWASM
{
    ...
    public class WXTextureEditorWindow : EditorWindow
    {
        ...
        public static void ReplaceBundle()
        {
            ...
            var fileCachedMd5Conf = UnityUtil.GetTextureFileCachedMd5Conf();
            Dictionary<string, WXFileCachedData> fileCachedData = new Dictionary<string, WXFileCachedData>();
            if (fileCachedMd5Conf.cachedDatas!=null) {
                foreach (var item in fileCachedMd5Conf.cachedDatas) {
                    if (!fileCachedData.ContainsKey(item.filePath))
                    {
                        fileCachedData.Add(item.filePath,item);
                    }
                }
            }
            ...
            manager = new TextureManager(new TextureManagerOption()
            {
                ...
                fileCachedData = fileCachedData,
                bundleDir = miniGameConf.CompressTexture.bundleDir
            });
            ...
            fileCachedMd5Conf.cachedDatas = manager.GetFileCachedMd5Datas();

            EditorUtility.SetDirty(fileCachedMd5Conf);
        }
        ...
        private static void OnReplaceEnd() {
            ...
            {
                if (!string.IsNullOrEmpty(sourceDataFile))
                {
                    var dstDataFile = miniGameConf.ProjectConf.DST + "/minigame/data-package/" + Path.GetFileName(sourceDataFile);
